...
HRESULT Renderer::Init(HWND hWnd)
{
	...
	m_pD3DDevice->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);		// テクスチャＵ値の繰り返し設定
	m_pD3DDevice->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);		// テクスチャＶ値の繰り返し設定
...
void Renderer::DrawBackBufferBegin()
{
	...
	m_pD3DDevice->Clear(0, NULL, (D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER), D3DCOLOR_RGBA(111, 111, 111, 255), 1.0f, 0);
	m_pD3DDevice->BeginScene();
}
